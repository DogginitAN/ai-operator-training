...
const COLUMNS: { key: TaskStatus; label: string; color: string; dotColor: string }[] = [
  { key: 'todo', label: 'To Do', color: 'text-surface-300', dotColor: 'bg-surface-300' },
...
const PRIORITY_COLORS = { high: 'bg-red-400', medium: 'bg-amber-400', low: 'bg-surface-300' };

function TaskCard({ task, onMove }: { task: Task; onMove: (id: string, status: TaskStatus) => void }) {
  return (
    <div className="bg-white rounded-xl border border-surface-200 p-4 card-lift shadow-card">
      <div className="flex items-start gap-2.5">
        <span className={`mt-2 w-2 h-2 rounded-full ${PRIORITY_COLORS[task.priority]} flex-shrink-0`} />
        <div className="flex-1 min-w-0">
          <h4 className="font-medium text-surface-900 text-sm leading-snug">{task.title}</h4>
          <p className="text-xs text-surface-300 mt-1.5 leading-relaxed">{task.description}</p>
        </div>
      </div>
          ...
          return (
            <button key={col.key} onClick={() => onMove(task.id, col.key)}
              className={`flex-1 py-1.5 text-[10px] font-mono font-semibold tracking-wide rounded-lg border transition-all duration-150 ${
                isActive
                  ? col.key === 'done' ? 'border-green-300 bg-green-50 text-green-600' : col.key === 'in_progress' ? 'border-blue-300 bg-blue-50 text-blue-600' : 'border-surface-300 bg-surface-100 text-surface-800'
                  : 'border-surface-200 bg-surface-50 text-surface-300 hover:border-surface-300 hover:text-surface-800'
              }`}
            >
              {col.label}
            </button>
        ...
        return (
          <div key={col.key}>
            <div className="flex items-center gap-2 mb-3 px-1">
              ...
              <span className="text-xs font-mono text-surface-300 ml-auto">{colTasks.length}</span>
            </div>
            <div className="space-y-2.5 min-h-[120px]">
              {colTasks.map(t => <TaskCard key={t.id} task={t} onMove={onMove} />)}
              {colTasks.length === 0 && (
                <div className="border-2 border-dashed border-surface-200 rounded-xl p-6 text-center text-xs text-surface-300">
