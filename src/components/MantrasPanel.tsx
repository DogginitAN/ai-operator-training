  ...
  return (
    <div className="bg-white rounded-2xl border border-surface-200 p-6 shadow-card">
      <div className="flex items-center gap-2 mb-4">
        ...
        <h3 className="text-xs font-mono font-semibold text-brand-600 uppercase tracking-wider">Core Principles</h3>
      </div>
      <div className="space-y-0">
        {mantras.map((m, i) => (
          <div key={i} className={`flex items-start gap-3 py-3 ${i < mantras.length - 1 ? 'border-b border-surface-100' : ''}`}>
            ...
            <p className="text-sm text-surface-800 leading-relaxed">{m}</p>
          </div>
        ))}
      </div>
    </div>
